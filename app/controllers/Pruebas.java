...
public class Pruebas extends Controller {
	...
	public static Promise<Result> parallel() {
		...
		final F.Promise<Long> yahoo = WS.url("http://www.yahoo.com").get()
				.map(timing);
		final F.Promise<Long> google = WS.url("http://www.google.com").get()
				.map(timing);
		final F.Promise<Long> bing = WS.url("http://www.bing.com").get()
				.map(timing);

		return F.Promise.sequence(yahoo, google, bing).map(
